...
preprocess.xlsin <- AddPFN(preprocess.xlsin, 
                           PFN("file:///scratch/r-home/advanced_workflow/GSE61533_HTSEQ_count_results.csv", "local"))
...
normalize1.sce <- File("normalize1-sce.RData")
...
var.out2.1 <- File("var.out.2.1.RData")
...
# Input file
cell.cycle.xls <- File("nbt.3102-S7.xlsx")
cell.cycle.xls <- AddPFN(cell.cycle.xls, 
                         PFN("file:///scratch/r-home/advanced_workflow/nbt.3102-S7.xlsx", "local"))
workflow <- AddFile(workflow, cell.cycle.xls)

# Specify input
cell.cycle.blocking <- Uses(cell.cycle.blocking, correlate.pairs.1.hsc, link = DAX3.Link$INPUT)
cell.cycle.blocking <- Uses(cell.cycle.blocking, cell.cycle.xls, link = DAX3.Link$INPUT)
